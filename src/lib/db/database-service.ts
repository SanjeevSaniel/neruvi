...
export class DatabaseService {
  ...
  async addMessage(
    ...
    try {
      console.log(
        `💾 Storing message: ${message.role} - ${message.content.length} characters`,
      );
      ...
      if (existingMessages.length >= 5) {
        ...
        console.log(
          `🗑️ Removing oldest message (${oldestMessageId}) to maintain 5-message limit`,
        );

        // Delete the oldest message and its chunks
        await this.db
          .delete(messageChunks)
          .where(eq(messageChunks.messageId, oldestMessageId));
        await this.db.delete(messages).where(eq(messages.id, oldestMessageId));
      }

      // Store content using tiered strategy
      const contentResult = await this.contentStorage.storeContent(
        message.content,
      );
      ...
      console.log(
        `✅ Message stored with ID: ${savedMessage.id} (${contentResult.storageType} storage)`,
      );
  ...
  private async storeMessageChunks(
    ...
    try {
      ...
      if (chunks.length > 0) {
        ...
        console.log(
          `📦 Stored ${chunks.length} chunks for message ${messageId}`,
        );
      }
    } catch (error) {
  ...
  async getConversationMessages(
    ...
    try {
      ...
      console.log('📨 Database getConversationMessages:', {
        ...
        assistantMessages: messagesData.filter((m) => m.role === 'assistant')
          .length,
      ...
      const reconstructedMessages = await Promise.all(
        messagesData.map(async (msgData) => {
          const messageChunksForMsg = chunks.filter(
            (c) => c.messageId === msgData.id,
          );
          ...
          const timestampDate = msgData.createdAt
      ...
      console.log(
        `📖 Retrieved ${reconstructedMessages.length} messages for conversation ${conversationId}`,
      );
  ...
  private async updateConversationMetadata(
    ...
    try {
      ...
      console.log(
        `📊 Updated conversation metadata: ${messageCount} messages, ${totalTokens} tokens`,
      );
    } catch (error) {
  ...
  async getUserUsage(
    userId: string,
  ): Promise<{ messageCount: number; limit: number; resetTime: Date } | null> {
    try {
   ...
   */
  async importFromSessionStorage(
    userId: string,
    sessionData: {
      conversations?: Array<{
        title?: string;
    ...
    try {
      ...
      if (
        ...
        Array.isArray(sessionData.conversations)
      ) {
      ...
      console.log(
        `📦 Migration completed: ${conversationsImported} conversations, ${messagesImported} messages`,
      );
